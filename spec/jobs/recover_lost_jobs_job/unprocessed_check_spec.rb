RSpec.describe RecoverLostJobsJob::UnprocessedCheck do
  describe "#call" do
    it "recovers unprocessed work over an hour old" do
      ...
      expect(ProcessContentChangeJob).to receive(:perform_async).with(work.id)
      subject.call
    end

    it "does not recover more recent unprocessed work" do
      create(:content_change, created_at: 59.minutes.ago, processed_at: nil)
      expect(ProcessContentChangeJob).not_to receive(:perform_async)
      subject.call
    end
    ...
    it "can also recover DigestRuns" do
      ...
      expect(WeeklyDigestInitiatorJob).to receive(:perform_async).with(work2.date.to_s)
