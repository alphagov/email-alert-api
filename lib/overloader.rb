...
private
  ...
  def generate_emails(sorted_lists)
    ...
    while generated_volume < requested_volume
      ...
      generated_volume += generate_matches(content_change, sorted_lists)
      ProcessContentChangeAndGenerateEmailsWorker.perform_async(content_change.id)
    end
  end

  def generate_matches(content_change, sorted_lists)
    generated_volume = 0
    total_lists = 0

    sorted_lists.each do |list|
      next if list_size(list).zero?
      next if generated_volume > requested_volume
